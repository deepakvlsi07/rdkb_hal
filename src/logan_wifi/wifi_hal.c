...
static int wifi_datfileRead(char *dat_file, char *param, char *output, int output_size)
{
...
static int wifi_datfileWrite(char *dat_file, struct params *list, int item_count)
{
    char cmd[MAX_CMD_SIZE]={'\0'};
    char buf[MAX_BUF_SIZE]={'\0'};

    for(int i=0;i<item_count;i++) {
        wifi_datfileRead(dat_file, list[i].name, buf, sizeof(buf));
        if (strlen(buf) == 0) //Insert
            snprintf(cmd, sizeof(cmd), "sed -i -e '$a %s=%s' %s", list[i].name, list[i].value, dat_file);
        else //Update
            snprintf(cmd, sizeof(cmd), "sed -i \"s/^%s=.*/%s=%s/\" %s", list[i].name, list[i].name, list[i].value, dat_file);
...
static void wifi_vap_status_reset()
{
	...
	if (access(VAP_STATUS_FILE, F_OK) != 0) {
		...
		snprintf(cmd, MAX_CMD_SIZE, "echo '' > %s", VAP_STATUS_FILE);
		_syscmd(cmd, ret_buf, sizeof(ret_buf));
	}
	...
	for (radio_idx = 0; radio_idx < MAX_NUM_RADIOS; radio_idx++)
    	for (bss_idx = 0; bss_idx < 5; bss_idx++) {
			snprintf(cmd, MAX_CMD_SIZE, "echo %s%d=0 >> %s", ext_prefix[radio_idx], bss_idx, VAP_STATUS_FILE);
			_syscmd(cmd, ret_buf, sizeof(ret_buf));
		}
...
INT wifi_setRadioCountryCode(INT radioIndex, CHAR *CountryCode)
{
	...
	if (!strlen(CountryCode))
		strncpy(CountryCode, "US", sizeof("US")); /*default set the code to US*/

	params.name = "country_code";
	params.value = CountryCode;

	snprintf(config_file, MAX_BUF_SIZE, "%s%d.conf", CONFIG_PREFIX, radioIndex);
...
INT wifi_setRadioChannel(INT radioIndex, ULONG channel)	//RDKB	//AP only
{
    ...
    sprintf(config_file_dat, "%s%d.dat", LOGAN_DAT_FILE, radioIndex);
...
INT wifi_setRadioDfsEnable(INT radioIndex, BOOL enable)	//Tr181
{
    ...
    wifi_datfileWrite(radioIndex, &dat, 1);
    WIFI_ENTRY_EXIT_DEBUG("Exiting %s:%d\n",__func__, __LINE__);
    return RETURN_OK;
}
...
INT wifi_getRadioExtChannel(INT radioIndex, CHAR *output_string) //Tr181
{
    ...
    char buf[64] = {0};
    wifi_band band;
    ULONG channel = 0, centr_channel = 0;
    UINT mode_map = 0;
...
INT wifi_getApName(INT apIndex, CHAR *output_string)
{
	...
	if (wifi_GetInterfaceName(apIndex, interface_name) != RETURN_OK) {
		...
		snprintf(output_string, IF_NAME_SIZE, "%s%d", ext_prefix[radio_idx], bss_idx);    // For wifiagent generating data model.
	} else
		snprintf(output_string, IF_NAME_SIZE, "%s", interface_name);
...
INT wifi_kickApAssociatedDevice(INT apIndex, CHAR *client_mac)
{
    ...
    snprintf(cmd, sizeof(cmd),"hostapd_cli -i %s disassociate %s", inf_name, client_mac);
    _syscmd(cmd, buf, sizeof(buf));
...
INT wifi_getApDevicesAssociated(INT apIndex, CHAR *macArray, UINT buf_size)
{
	...
	snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s list_sta", interface_name);
...
int wifihal_interfacestatus(CHAR *wifi_status, CHAR *interface_name)
{
    ...
    snprintf(cmd, MAX_CMD_SIZE, "ifconfig %s | grep RUNNING | tr -s ' ' | cut -d ' ' -f4 | tr -d '\\n'",
		interface_name);
