...
INT wifi_getMaxRadioNumber(INT *max_radio_num)
{
	...
	if (res == 0 && buf[0] != '0') {
		wifi_debug(DEBUG_ERROR, "Unexpected strtol fail\n");
		return RETURN_ERR;
	}
...
INT wifi_getRadioChannelStats2(INT radioIndex, wifi_channelStats2_t *outputChannelStats2)
{
	...
	if ((buf[0] != '\0') && (outputChannelStats2->ch_Max80211Rssi == 0))
		wifi_debug(DEBUG_ERROR, "Unexpected strtol fail\n");
...
INT wifi_getBasicTrafficStats(INT apIndex, wifi_basicTrafficStats_t *output_struct)
{
	...
	res = snprintf(cmd, sizeof(cmd),
		"hostapd_cli -i %s list_sta | wc -l | tr -d '\n'", interface_name);
	if (os_snprintf_error(sizeof(cmd), res)) {
		wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
		return RETURN_ERR;
	}
...
INT wifi_getWifiTrafficStats(INT apIndex, wifi_trafficStats_t *output_struct)
{
	...
	res = snprintf(buf, sizeof(buf),
		"cat /tmp/SSID_Stats.txt | grep 'RX packets' | tr -s ' ' | cut -d ':' -f3 | cut -d ' ' -f1");
	if (os_snprintf_error(sizeof(buf), res)) {
		wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
		return RETURN_ERR;
	}
	...
	if ((Value[0] != '\0') && (output_struct->wifi_ErrorsReceived == 0)) {
		wifi_debug(DEBUG_ERROR, "Unexpected strtoul fail\n");
		return RETURN_ERR;
	}

	res = snprintf(buf, sizeof(buf),
		"cat /tmp/SSID_Stats.txt | grep 'TX packets' | tr -s ' ' | cut -d ':' -f3 | cut -d ' ' -f1");
	if (os_snprintf_error(sizeof(buf), res)) {
		wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
		return RETURN_ERR;
	}
	...
	if ((Value[0] != '\0') && (output_struct->wifi_ErrorsSent == 0)) {
		wifi_debug(DEBUG_ERROR, "Unexpected strtoul fail\n");
		return RETURN_ERR;
	}

	res = snprintf(buf, sizeof(buf),
		"cat /tmp/SSID_Stats.txt | grep 'RX packets' | tr -s ' ' | cut -d ':' -f4 | cut -d ' ' -f1");
	if (os_snprintf_error(sizeof(buf), res)) {
		wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
		return RETURN_ERR;
	}
	...
	if ((Value[0] != '\0') && (output_struct->wifi_DiscardedPacketsReceived == 0)) {
		wifi_debug(DEBUG_ERROR, "Unexpected strtoul fail\n");
		return RETURN_ERR;
	}

	res = snprintf(buf, sizeof(buf),
		"cat /tmp/SSID_Stats.txt | grep 'TX packets' | tr -s ' ' | cut -d ':' -f4 | cut -d ' ' -f1");
	if (os_snprintf_error(sizeof(buf), res)) {
		wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
		return RETURN_ERR;
	}
	...
	if ((Value[0] != '\0') && (output_struct->wifi_DiscardedPacketsSent == 0)) {
		wifi_debug(DEBUG_ERROR, "Unexpected strtoul fail\n");
		return RETURN_ERR;
	}
...
INT wifi_getIndexFromName(CHAR *inputSsidString, INT *output_int)
{
	...
	if (strlen(buf)) {
		...
		*output_int = strtoul(apIndex_str, NULL, 10);
		if (*output_int == 0 && apIndex_str[0] != '0') {
			wifi_debug(DEBUG_ERROR, "Unexpected strtol fail\n");
			return RETURN_ERR;
		}
		return RETURN_OK;
...
INT wifi_setApSecurityReset(INT apIndex)
{
	...
	if (access(wpa_psk_file, F_OK) != 0) {
		res = snprintf(cmd, sizeof(cmd), "touch %s", wpa_psk_file);
		if (os_snprintf_error(sizeof(cmd), res)) {
			wifi_debug(DEBUG_ERROR, "Unexpected snprintf fail\n");
			return RETURN_ERR;
		}
...
INT wifi_getTWTsessions(INT ap_index, UINT maxNumberSessions, wifi_twt_sessions_t *twtSessions, UINT *numSessionReturned)
{
	...
	*numSessionReturned = strtol(buf, NULL, 10) - 1;
	...
	while((fgets(line, sizeof(line), f)) != NULL) {
		...
		if ((tmp[0] != '\0') && (mantissa == 0))
			continue;
		tmp = strtok(NULL, " ");
		duration = strtol(tmp, NULL, 10);
		if ((tmp[0] != '\0') && (duration == 0))
			continue;
